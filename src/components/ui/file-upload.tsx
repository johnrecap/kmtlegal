...
/**
 * KMT File Upload — visual/behavioral port of the official Aceternity UI
 * File Upload (https://ui.aceternity.com/components/file-upload), vendored
 * 2026-09-19. GridPattern, motion variants, file cards, single-shot
 * click-to-browse (`noClick` dropzone + hidden input) are official.
 *
 * Documented deviations (no new package installs — the repo carries
 * pre-existing uncommitted package.json/package-lock.json hunks that an
 * `npm install` would churn; see Phase 08 notes):
 * - `IconUpload` from `@tabler/icons-react` aliased to the declared
 *   `lucide-react` `Upload` (same icon semantics; Sidebar Phase 07
 *   precedent).
 * - `useDropzone` (`react-dropzone`: `multiple: false`, `noClick: true`,
 *   `onDrop`, `onDropRejected: console.log`) re-implemented with native
 *   HTML5 drag-and-drop exposing the identical surface: `isDragActive`
 *   visual state, drop → `File[]`, rejected → `console.log`. Click-to-browse
 *   via the hidden input is preserved.
 * - Added optional `accept` (applied to the hidden input AND used to filter
 *   dropped files) so the caller can enforce its verified accept list. The
 *   native `accept` attribute only guides the picker dialog; drops are
 *   filtered to the same list instead of inventing new constraints.
 */
export const FileUpload = ({
  ...
  const handleFileChange = (newFiles: File[]) => {
    ...
    const rejected = newFiles.filter((file) => !isAccepted(file));
                ...
                >
                  <div className="flex w-full items-center justify-between gap-4">
                    <motion.p
                      ...
                      animate={{ opacity: 1 }}
                      layout
                      className="max-w-[12rem] truncate text-base text-neutral-700 sm:max-w-xs dark:text-neutral-300"
                    >
                      {file.name}
                    </motion.p>
                    <motion.p
