/**
 * Animate UI Base Menu primitive (official source:
 * https://animate-ui.com/r/primitives-base-menu.json, fetched 2026-09-19
 * for Phase 09). OWNER-APPROVED ANIMATE UI DEPENDENCY-NAMESPACE ADAPTATION
 * — body otherwise byte-identical:
 * - `@base-ui-components/react/menu` → `@base-ui/react/menu` (Base UI
 *   renamed the package namespace; `@base-ui/react@1.8.0` is installed).
 * This is NOT verbatim registry source, NOT a Radix substitution, NOT a
 * custom Menu. Registry deps vendored: highlight effect
 * (`primitives/effects/highlight`), `use-data-state` (`@/hooks`),
 * `use-controlled-state`, `get-strict-context`.
 */
'use client';
...
function MenuSubmenuTrigger({
  ...
  return (
    <MenuPrimitive.SubmenuTrigger
      // NOTE (KMT): ref-type cast only — React 18 RefObject (nullable
      // current) vs Base UI LegacyRef; zero runtime effect.
      ref={highlightedRef as React.Ref<HTMLElement>}
      label={label}
      id={id}
...
function MenuItem({
  ...
  return (
    <MenuPrimitive.Item
      // NOTE (KMT): ref-type cast only — see SubmenuTrigger above.
      ref={highlightedRef as React.Ref<HTMLElement>}
      label={label}
