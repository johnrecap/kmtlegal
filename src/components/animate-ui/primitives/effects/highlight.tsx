...
const HighlightContext = React.createContext<
  // NOTE (KMT): official `eslint-disable no-explicit-any` dropped — this
  // repo's ESLint setup does not provide that rule (same as
  // `use-controlled-state`); `any` here is registry-verbatim.
...
function HighlightItem<T extends React.ElementType>({
  ...
  const refCallback = React.useCallback((node: HTMLElement | null) => {
    // NOTE (KMT): MutableRefObject cast — React 18 types mark
    // RefObject.current readonly (same pattern as primitives/animate/slot).
