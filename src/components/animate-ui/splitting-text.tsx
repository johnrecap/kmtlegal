 ...
 * Source: apps/www/registry/primitives/texts/splitting/index.tsx
 * Local adaptation: asChild/Slot support removed (unused here).
 *
 * RTL note: use type="words" or "lines" for Arabic — never "chars"
 * (per-character animation breaks Arabic cursive shaping).
 */
    ...
    >
      {tokens.map((tok, wi) => {
        ...
        return (
          ...
          >
            {chars.map((ch, ci) => (
              <motion.span key={`ch-${wi}-${ci}`} variants={itemVariants} style={{ display: "inline-block", whiteSpace: "pre" }}>
                {ch}
              </motion.span>
            ))}
          </motion.span>
