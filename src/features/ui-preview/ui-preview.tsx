...
export function UiPreview() {
  ...
  const editStage = (next: BookingStage) => { setStatus("idle"); setStage(next); dialog.current?.close(); window.setTimeout(() => document.querySelector<HTMLElement>(`[data-preview-stage="${next}"]`)?.focus(), 0); };
...
function BookingForm({ content, copy, serviceSlug, method, details, slot, unavailableSlot, stage, status, setService, setMethod, setDetails, setSlot, setUnavailableSlot, setStage, setStatus }: { content: ReturnType<typeof getPublicContent>; copy: ReturnType<typeof getUiPreviewCopy>["booking"]; serviceSlug: string; method: Mode; details: string; slot: string; unavailableSlot: string; stage: BookingStage; status: "idle" | "conflict" | "confirmed"; setService: (value: string) => void; setMethod: (value: Mode) => void; setDetails: (value: string) => void; setSlot: (value: string) => void; setUnavailableSlot: (value: string) => void; setStage: (value: BookingStage) => void; setStatus: (value: "idle" | "conflict" | "confirmed") => void }) {
  ...
  return <section className={styles.conversation} aria-label={copy.title}>
    ...
    {stage === "service" ? <div className={styles.quickChoices}>{content.legalServices.map((item, index) => <button autoFocus={index === 0} data-preview-stage="service" key={item.slug} onClick={() => { setService(item.slug); reset(); setStage(details ? (slot ? "review" : "slot") : "details"); }} type="button">{item.title}</button>)}</div> : <p className={styles.userMessage}>{service.title}<button onClick={() => setStage("service")} type="button">{copy.edit}</button></p>}
    {stage !== "service" ? <><p className={styles.assistantMessage}>{copy.method}</p>{stage === "method" ? <div className={styles.quickChoices}>{(["ONLINE", "PHONE", "OFFICE"] as Mode[]).map((item, index) => <button autoFocus={index === 0} data-preview-stage="method" key={item} onClick={() => { setMethod(item); reset(); setStage(details ? (slot ? "review" : "slot") : "details"); }} type="button">{copy[item.toLowerCase() as "online" | "phone" | "office"]}</button>)}</div> : <p className={styles.userMessage}>{copy[method.toLowerCase() as "online" | "phone" | "office"]}<button onClick={() => setStage("method")} type="button">{copy.edit}</button></p>}</> : null}
...
function SummaryItem({ label, value, edit, onEdit }: { label: string; value: string; edit: string; onEdit: () => void }) {
  return <div><dt>{label}<button onClick={onEdit} type="button">{edit}</button></dt><dd>{value}</dd></div>;
}
