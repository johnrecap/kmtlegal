...
export async function ClientAccountSetupPage({
  locale,
  searchParams
}: ClientAccountSetupPageProps) {
  ...
  if (context && context.locale !== locale) {
    const canonicalPath = context.locale === "ar"
      ? "/ar/client-account/setup"
      : "/client-account/setup";
    redirect(`${canonicalPath}?token=${encodeURIComponent(token)}`);
  }
  ...
  return (
    ...
    >
      <main
        className="mx-auto min-h-[68vh] max-w-[1060px] px-4 py-14 sm:px-6 lg:px-10"
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_0.9fr] lg:items-start">
          <div className="rounded-[1.75rem] border border-kmt-gold/35 bg-[linear-gradient(145deg,#17110a_0%,#090b0d_54%,#020202_100%)] p-6 shadow-[0_34px_120px_-68px_rgba(183,134,64,0.58)] sm:p-8">
            ...
            <p className="mt-4 max-w-2xl text-base leading-8 text-amber-50/78">
              {context ? copy.description : copy.expiredDescription}
            </p>

            {!context ? (
              <div className="mt-8 flex flex-wrap gap-3">
                <Link className={primaryActionClasses()} href={`/login?next=/client&locale=${locale}`}>
              ...
              <div className="mt-8 rounded-2xl border border-kmt-gold/25 bg-black/25 p-5">
                ...
                <Link
                  className={cn(primaryActionClasses(), "mt-5")}
...
function ConsultationSummary({
  ...
  const rows = [
    ...
    { label: copy.consultationSummary, value: context.consultation.summary },
    {
      ...
      value: context.consultation.appointment
        ? formatDateTime(context.consultation.appointment.startsAt, locale)
        : content.bookingChat.noSlot
    }
  ];
