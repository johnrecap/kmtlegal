...
export default async function PaymentReceiptRoute({
  ...
  if (!receipt) notFound();
  if (receipt.locale === "en") {
    redirect(`/payment/consultation/receipt?${receiptQuery(params ?? {})}`);
  }

  return <ConsultationPaymentReceiptPage locale="ar" params={params} receipt={receipt} />;
}
