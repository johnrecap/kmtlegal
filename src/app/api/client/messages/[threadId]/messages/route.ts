...
import { conversationMessageCreateSchema, replyClientConversation } from "@/server/conversations/conversation-service";
...
export async function POST(request: Request, { params }: ClientMessageRouteProps) {
  ...
  try {
    ...
    const result = await replyClientConversation({
    ...
    return errorToResponse(error, requestId, { routeGroup: "client.messages.reply", method: "POST" });
  }
}
