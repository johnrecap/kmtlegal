...
import { assignAdminClient, assignClientSchema } from "@/server/admin/client-crm-service";
...
export async function POST(request: Request, { params }: ClientActionRouteProps) {
  ...
  try {
    ...
    const body = await parseJsonRequest(request, assignClientSchema, "Client assignment payload is invalid.");
    const client = await assignAdminClient({
