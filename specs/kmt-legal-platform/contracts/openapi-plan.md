...
| POST | `/api/public/consultations` | Legacy manual-review consultation request; disabled for the public booking page so the assistant cannot be bypassed | Guest | consultation.create.public |
| POST | `/api/public/consultations/assistant` | AI-assisted consultation booking and verified appointment inquiry; enabled when `consultation.booking.mode=AI_CHAT_PAID` or `AI_CHAT_FREE` | Guest | consultation.create.public |
