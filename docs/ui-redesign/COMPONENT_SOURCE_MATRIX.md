...
| 31 | File Upload | Aceternity UI | https://ui.aceternity.com/components/file-upload | CURRENT | Installed Phase 08: `src/components/ui/file-upload.tsx` (official visual/behavior port; `IconUpload` aliased to declared `lucide-react`, `useDropzone` re-implemented with native HTML5 drag-and-drop exposing the identical surface, plus an `accept` prop enforcing the caller accept list — no new package installs, documented in `08_CLIENT_PAGES.md`; one responsive tweak: file-name `max-w-xs` → `max-w-[12rem] sm:max-w-xs` against 390px overflow) | Client files upload; admin documents upload | 08, 09, 11 |
