...
| 03 | Sheet | Animate UI | https://animate-ui.com/docs/components/radix/sheet | CURRENT | `src/components/animate-ui/components/radix/sheet.tsx` | Public mobile drawer; client mobile nav (client mobile filters NOT APPLICABLE per Phase 07 owner ruling); admin mobile nav + mobile forms/filters | 02, 07, 08, 09, 10, 11 |
...
| 26 | Sidebar | Aceternity UI | https://ui.aceternity.com/components/sidebar | CURRENT | Installed Phase 07: `src/components/ui/sidebar.tsx` (official source verbatim; `@tabler/icons-react` menu icons aliased to declared `lucide-react` dep — single import-line deviation, documented in `07_CLIENT_FOUNDATIONS.md`) | Client desktop navigation; admin desktop sidebar | 07, 09 |
...
| 29 | Menu | Animate UI | https://animate-ui.com/docs/components/base/menu | CURRENT — OWNER-APPROVED ANIMATE UI DEPENDENCY-NAMESPACE ADAPTATION | `primitives/base/menu.tsx` + `components/base/menu.tsx` (official source; `@base-ui-components/react/menu` → `@base-ui/react/menu`, documented in-file; registry deps `hooks/use-data-state` + `primitives/effects/highlight` vendored verbatim; two trivial mechanical ref-type casts for React 18 types — `highlightedRef as React.Ref<HTMLElement>` at 4 call sites + `MutableRefObject` cast in highlight `refCallback` following the `primitives/animate/slot.tsx` precedent — zero runtime effect, documented in `09_ADMIN_FOUNDATIONS.md`); kit `src/components/admin/admin-menu.tsx` (`AdminRowActions`) | Admin row actions; contact-message actions; users row actions | 09, 10, 11 |
| 30 | Popover | Animate UI | https://animate-ui.com/docs/components/base/popover | CURRENT — OWNER-APPROVED ANIMATE UI DEPENDENCY-NAMESPACE ADAPTATION | `primitives/base/popover.tsx` + `components/base/popover.tsx` (official source; `@base-ui-components/react/popover` → `@base-ui/react/popover`, documented in-file; body otherwise byte-identical); bell migrated `details/summary` → `Popover/PopoverTrigger/PopoverPanel` in `admin-notification-popover.tsx` (polling + unread + mark-read + links preserved; panel surface bound via the designed `className` extension point to `bg-white text-kmt-ink border-kmt-border` because official `bg-popover` utilities are dead in this repo's theme — no popover tokens; token completion deferred, needs owner coordination since `tokens.ts`/`tailwind.config.ts` carry foreign hunks) | Admin advanced desktop filters; notification bell | 09, 10 |
| 31 | File Upload | Aceternity UI | https://ui.aceternity.com/components/file-upload | CURRENT — OWNER-APPROVED ADAPTATION | `src/components/ui/file-upload.tsx` (official visual/behavior port; `IconUpload` aliased to declared `lucide-react`, `useDropzone` re-implemented with native HTML5 drag-and-drop exposing the identical surface, plus an `accept` prop enforcing the caller accept list — no new package installs, documented in `08_CLIENT_PAGES.md`; one responsive tweak: file-name `max-w-xs` → `max-w-[12rem] sm:max-w-xs` against 390px overflow; pre-Phase-09 ruling: do NOT describe as verbatim, do NOT replace, do NOT install `react-dropzone`, reuse this SAME file in admin — no second implementation) | Client files upload; admin documents upload | 08, 09, 11 |
| 32 | Pagination | shadcn/ui | https://ui.shadcn.com/docs/components/pagination | LOCKED (admin only) | Local `ui/pagination.tsx` is gallery-only; client lists render full lists with no pagination behavior (NOT APPLICABLE per Phase 07 owner ruling — no adapter built, no `?page=` state, no page wiring; recorded in `07_CLIENT_FOUNDATIONS.md`) | Admin list pagination Phase 09/10 (replaces hand-rolled prev/next links); locked stand-by for client if a real pagination requirement appears | 09, 10 |
