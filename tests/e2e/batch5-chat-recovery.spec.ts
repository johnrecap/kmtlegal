...
for(const locale of ["en","ar"] as const){
 ...
 test(`${locale}: public language navigation retains contact and unsent text without persisting a confirmation`,async({page})=>{
  ...
  await page.locator('[data-testid="public-language-switch"]:visible').click();await expect(input).toHaveValue("Unsent question");await expect(input).toBeEnabled();await submit.click();await expect(input).toBeEnabled();expect(received.draft.phone).toBe(contact.phone);expect(received.draft.fullName).toBe(contact.fullName);expect(received.locale).toBe(locale==="ar"?"en":"ar");expect(received.confirmBooking).not.toBe(true);expect(received.selectedSlot).toBe("");
...
for(const locale of ["en","ar"] as const){
 ...
 test(`${locale}: language retains reviewed slot and completed booking reference without auto-booking`,async({page})=>{
  ...
  await page.locator('[data-testid="public-language-switch"]:visible').click();await expect.poll(()=>requests.length).toBe(2);expect(requests[1].selectedSlot).toBe(draft.startsAt);expect(requests[1].confirmBooking).not.toBe(true);expect(requests[1].locale).toBe(other);await expect(page.getByTestId("booking-confirm-booking")).toBeVisible();
  ...
  await page.locator('[data-testid="public-language-switch"]:visible').click();await expect(page.getByTestId("booking-chat-log")).toContainText("CONS-1234ABCD");await expect(page.getByTestId("booking-confirm-booking")).toHaveCount(0);expect(requests).toHaveLength(3);
 });
 test(`${locale}: language preserves inquiry and blocks oversized unsent transfer`,async({page})=>{
  await page.goto(path);await page.getByTestId(`booking-language-${locale}`).click();await page.getByTestId("booking-quick-inquiry").click();const input=page.locator('input[name="chatMessage"]');await input.fill("My pending question");await page.locator('[data-testid="public-language-switch"]:visible').click();await expect(page.getByTestId("booking-chat-log")).toContainText(getPublicContent(other).bookingChat.inquiryPrompt);await expect(input).toHaveValue("My pending question");
  const long="x".repeat(20001);await input.fill(long);const before=page.url();await page.locator('[data-testid="public-language-switch"]:visible').click();await expect(page).toHaveURL(before);await expect(input).toHaveValue(long);expect(await page.evaluate(()=>sessionStorage.getItem("kmt.booking.language-transfer"))).toBeNull();
 });
}
test("oversized language transfer stays on the current page with its complete unsent text",async({page})=>{
 await page.goto("/book-consultation");await page.getByTestId("booking-language-en").click();const input=page.locator('input[name="chatMessage"]');const long="x".repeat(20001);await input.fill(long);const before=page.url();await page.locator('[data-testid="public-language-switch"]:visible').click();await expect(page).toHaveURL(before);await expect(input).toHaveValue(long);await expect(page.getByTestId("booking-chat-log")).toContainText(getPublicContent("en").bookingChat.languageTransferTooLarge);expect(await page.evaluate(()=>sessionStorage.getItem("kmt.booking.language-transfer"))).toBeNull();
 const shorter="x".repeat(19000);await input.fill(shorter);await page.locator('[data-testid="public-language-switch"]:visible').click();await expect(page).toHaveURL(/\/ar\/book-consultation$/);await expect(input).toHaveValue(shorter);
});
